...
const DB_PATH = path.resolve(process.env.DB_PATH || './data/reviews.db');
...
const initDatabase = async () => {
  ...
  // ── Tabla de reseñas ──
  rawDb.run(`CREATE TABLE IF NOT EXISTS reviews (
    id TEXT PRIMARY KEY,
  ...
  // ── Índices ──
  rawDb.run('CREATE INDEX IF NOT EXISTS idx_reviews_product ON reviews(product_id)');
  rawDb.run('CREATE INDEX IF NOT EXISTS idx_reviews_user ON reviews(user_id)');
  rawDb.run('CREATE INDEX IF NOT EXISTS idx_reviews_rating ON reviews(rating)');
  rawDb.run('CREATE INDEX IF NOT EXISTS idx_reviews_approved ON reviews(is_approved)');
  ...
  console.log('[reviews-service] Database initialized:', DB_PATH);
};
